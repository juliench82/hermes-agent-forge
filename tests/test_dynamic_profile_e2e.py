"""Domain-agnostic orchestration coverage for Hermes onboarding."""
...
class DomainAgnosticDiscoveryTests(unittest.TestCase):
    def test_arbitrary_customer_context_reaches_hermes(self):
    ...
    def test_different_context_can_return_different_valid_team(self):
        ...
        self.assertEqual(len(team["profiles"]), 5)

    def test_semantically_invalid_response_is_retried_and_rejected(self):
        invalid = '{"team_size": 4, "profiles": []}'
        hermes = FakeHermes([result(invalid), result(invalid)])
        ...
        self.assertIn("invalid", hermes.calls[1][3].lower())


class DomainAgnosticProvisioningTests(unittest.TestCase):
    def test_invalid_team_is_rejected_without_side_effects(self):
        calls = []
        ...
        invalid = {"team_size": 3, "profiles": [{"name": "unsafe name", "description": "x", "skills": ["x"]}]}
    ...
    def test_valid_team_provisions_and_renders_soul(self):
        calls = []
        ...
        team = {"team_size": 3, "profiles": [{"name": "coordination", "description": "Coordinate the workflow.", "skills": ["planning"]}, {"name": "execution", "description": "Complete approved work.", "skills": ["execution"]}, {"name": "review", "description": "Verify outcomes.", "skills": ["verification"]}]}
        with tempfile.TemporaryDirectory() as directory:
            ...
            soul = Path(directory, "coordination", "SOUL.md").read_text(encoding="utf-8")
        self.assertEqual(len(created), 3)
        self.assertIn("# coordination", soul)
