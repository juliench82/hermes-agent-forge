...
- `payload_ref` points to the full artifact in the Obsidian vault, not inline content. This keeps BUZZ messages compact per `shared/context-policy.md`.
- `needs_approval: true` freezes the chain until the orchestrator relays a user approval, per `shared/safety-gates.md`.
